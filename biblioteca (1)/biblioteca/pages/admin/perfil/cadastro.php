...
if (isset($_POST["cadastrar"])) {
   criarPerfilAction($_POST["descricao"]); 
}

if (isset($_POST["alterar"])) {
   alterarPerfilAction($_POST["cod_perfil"],
                       $_POST["descricao"]); 
}

$editMode = null;
if (isset($_GET['id'])) {
   $perfil = buscarPerfilAction($_GET['id']);
   if (empty($perfil)) {
...
?>
         <div class="container-fluid">
            ...
            <div class="row" style="overflow-y: auto; height:100vh;">
               <div class="col" > 
                  <h1><?=$editMode ? "Editar Perfil de Usuário" : "Cadastrar Perfil de Usuário"?></h1>
                  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                     <div class="form-group" >
                        <label for="descricao">Descrição*</label>
                        <input value="<?=$editMode ? htmlspecialchars($perfil['descricao']) : ""?>" type="text" class="form-control" name="descricao" maxlength="45" required><br/>
                     </div>
                     <div class="form-group" >
<?php if ($editMode): ?>
                        <input type="hidden" name="cod_perfil" id="cod_perfil" value="<?=$cod_perfil?>" />
<?php endif; ?>
